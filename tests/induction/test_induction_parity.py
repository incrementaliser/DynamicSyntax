...
_DATA = _REPO / "data" / "induction-test" / "one.txt"
_GRAMMAR = _REPO / "resources" / "2025-seed-grammar"
_HYP_REF = _REPO / "data" / "induction-test" / "hypotheses-reference.txt"


def _gold_from_one_txt() -> TTRRecordType:
    """Parse gold TTR from ``data/induction-test/one.txt``."""
    data = _DATA.read_text(encoding="utf-8")
...
def test_open_a_door_hypothesiser_trace_steps_1_to_18() -> None:
    """Replay ``hypothesise_once`` navigation through Java trace steps 1–18 entry."""
    rt = _gold_from_one_txt()
    hyp = TTRHypothesiser(str(_GRAMMAR))
    hyp.load_training_example("open a door", rt)
