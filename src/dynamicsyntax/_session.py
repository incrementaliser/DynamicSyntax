...
def _bundled_grammar_node(canonical_id: str) -> Any:
    """Return a traversable ``grammars/<canonical_id>`` node, or ``None`` if missing."""
    root = resources.files("dynamicsyntax")
    node = root / "grammars" / canonical_id
...
def load_grammar(grammar: str | Path, *, repairing: bool = False) -> None:
    ...
    node = _bundled_grammar_node(canonical)
    if node is None:
        known = ", ".join(get_grammars()) or "(none)"
        raise FileNotFoundError(
            f"unknown bundled grammar {grammar!r} (resolved {canonical!r}); "
            f"known: {known}",
        )
...
def resolved_grammar_path(grammar: str | Path) -> Iterator[Path]:
    ...
    if node is None:
        known = ", ".join(get_grammars()) or "(none)"
        raise FileNotFoundError(
            f"unknown bundled grammar {grammar!r} (resolved {canonical!r}); known: {known}",
        )
