...
class TTRHypothesiser(Hypothesiser):
    ...
    def initialise(self) -> None:
        ...
        for inc_list in inc_set:
            ...
            for tree in trees:
                ...
                merged_inc = cur_target.merge_tree_put_all(tree_hyp.get_tree())
                merged_nonhead = cur_nonhead.merge_tree_put_all(tree_hyp.get_tree())
    ...
    def apply_tree_hypotheses_entity(self) -> None:
        ...
        for inc in inc_set:
            ...
            for tree in trees:
                ...
                child = DAGInductionTuple(self.state.get_current_tuple().get_tree().clone())
                merged_inc = cur.get_target_tree().merge_tree_put_all(tree_hyp.get_tree())
                merged_nonhead = cur.get_non_head_target().merge_tree_put_all(tree_hyp.get_tree())
                child_added = self.state.add_child(child, tree_hyp, None)
                child_added.set_target(merged_inc)
                child_added.set_non_head_target(merged_nonhead)
    ...
    def apply_tree_hypotheses_event(self) -> None:
        ...
        for inc in inc_set:
            ...
            for i, tree in enumerate(trees):
                ...
                merged_inc = cur.get_target_tree().merge_tree_put_all(tree)
                merged_nonhead = cur_nonhead.merge_tree_put_all(non_head_tree)
                child_added = self.state.add_child(child, tree_hyp, None)
                child_added.set_target(merged_inc)
                child_added.set_non_head_target(merged_nonhead)
    ...
    def apply_optional_grammar(self, target: Tree | None = None) -> None:
        """Apply optional grammar actions gated by maximal-semantics subsumption (Java ``applyOptionalGrammar``)."""
        if self.target_type is None:
            return
        for action in self.optional_grammar.values():
            ...
            try:
                if action.backtracks_on_success():
