...
from dylan.induction.em_learner.learn_progress import LearnProgressReporter
...
class WordLearner(Generic[T]):
    ...
    def __init__(
        ...
        self._progress_reporter: LearnProgressReporter | None = None
    ...
    def learn(self, *, show_progress: bool = False) -> None:
        ...
        with LearnProgressReporter(size, enabled=show_progress) as reporter:
            self._progress_reporter = reporter
            try:
                i = 0
                while self.learn_once():
                    reporter.finish_example()
                    i += 1
                    logger.info("So far processed: %d of %d", i, size)
            finally:
                self._progress_reporter = None

    def _begin_example(self, label: str) -> None:
        """Notify the progress reporter that a new corpus example is starting."""
        if self._progress_reporter is not None:
            self._progress_reporter.begin_example(label)
